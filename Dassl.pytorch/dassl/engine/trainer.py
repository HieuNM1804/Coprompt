...
class TrainerX(SimpleTrainer):
    ...
    def run_epoch(self):
        ...
        pbar = tqdm(self.train_loader_x, desc=f"Epoch [{self.epoch + 1}/{self.max_epoch}]")
        for self.batch_idx, batch in enumerate(pbar):
            data_time.update(time.time() - end)
            loss_summary = self.forward_backward(batch)
            batch_time.update(time.time() - end)
            losses.update(loss_summary)

            pbar.set_postfix({"loss": f"{losses.meters['loss'].avg:.4f}"} if "loss" in losses.meters else {})
